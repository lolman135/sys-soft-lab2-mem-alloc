...
int main(void) {
    ...
    allocator_t* alloc = create_allocator();
    if (!alloc) {
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        if (r < 45 && active_count < MAX_BLOCKS) {
            ...
            mem_free(alloc, p, blocks[idx].requested_size);
            ...
            void* new_ptr = mem_realloc(alloc, old_ptr, old_sz, new_sz);
    ...
    for (int i = 0; i < active_count; i++) {
        ...
        mem_free(alloc, blocks[i].ptr, blocks[i].requested_size);
    }
